...
The `.env.local` file contains the API keys and passwords which are sensitive things no random user should be able to see.
If we were to commit this to the repo, it would be in the git history permanently which is why we should not commit this.

## 2. Why are GitHub Secrets safer than plain environment variables?

GitHub secrets are encrypted while plain environment variables are not. GitHub secrets are hidden in log files while plain environment variables can possibly appear in the CI output. Most importantly, GitHub secrets are never stored in your repo while plain environment variables can be.

## 3. What happens if you hardcode a value in your YML file?
...
Similar to the previous answers, this would make the secret visible to anyone who can see the repository, it would permanently be stored in Git history, and it will print in CI logs.
